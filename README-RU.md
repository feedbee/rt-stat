...
Проект находится в разработке. Последняя версия 0.3.1.
...
Запросы и ответы состоят из комманд и опциональных аргументов. Имена комманд являеются регистро-независимыми. Протокол ненаправленный: и запросы, и ответы могут слаться как клиентом, так и сервером (но наборы команд для сервера и клиента разные). В случае успешного выполнения команды, которая не требует ответа, ответ не отправляется (на данный момент ответа требуют только комманды `version` и `ping`). В случае ошибки приходит ответ с командой `error` и текстом ошибки на английском языке в виде аргумента.
...
**Команды клиента (отправляются на сервер):**
...
- `pong` — ответ на запрос о состоянии соединения;
- `quit` или `exit` — разрыв соединения;
- `version` — запрос версии сервера.

**Конманды сервера (отправляются клиенту):**
...
- `ping` - проверка состояния соединения; требует ответа в течение 30 секунд, иначе соединение будет закрыто со стороны сервера;
- `welcome` — приветственное сообщения сервера после подключения (первый аргумент — имя и версия сервера, второй — список поддерживаемых фич (в настоящее время только `ping`)).
